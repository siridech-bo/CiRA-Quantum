...
| 6 | ✅ shipped | Async job queue via Redis-RQ. Default launcher is `ThreadedJobLauncher` (no Redis required) — opt in with `USE_REDIS_QUEUE=1` + `REDIS_URL=...`. `RQJobLauncher` enqueues onto a Redis-backed queue and a separate `python worker.py` process picks jobs off. Event bus migrates to Redis Streams so SSE in Flask sees events emitted from the worker process. `pip install -e ".[queue]"` pulls `rq` + `redis`; tests use `fakeredis`. Windows workers use `SimpleWorker` (no fork) with a no-op death-penalty class. |
| 7 | ✅ shipped | Admin read-only views — `GET /api/admin/{users,jobs,overview}` gated by `@admin_required`; frontend `AdminPage.vue` at `/admin` has three tabs (Overview headline counters, per-user table with stored BYOK provider names + job counts, paginated cross-user job list with status filter + drill-into-detail). "Admin" link appears in the MainApp app-bar when `auth.user.role === 'admin'`; non-admins bounce back to `/` via the existing `requiresAdmin` router guard. No mutation endpoints in this first cut — admins still manage their own creds via the normal `/api/keys` routes; cross-user mutation requires a separate review and lands as Phase 7B. |
