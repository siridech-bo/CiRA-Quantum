...
LSTM (${\sim}1.6\times$) and the ESN (${\sim}3.7\times$), confirmed across three seeds.
We therefore report no blanket quantum advantage, but a principled,
...
MG $h{=}10$ (3) & $0.0304$     & $\mathbf{0.00155\pm0.00018}$ & --- \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
scale; error bars are the multi-seed s.d.); the $\tau\!\to\!0$ ablation collapses the
learned model to a mean predictor ($\approx1.0$), so the gain is carried by the
quantum reservoir.}
\end{figure}

\begin{figure}[t]
\includegraphics[width=\columnwidth]{figP4_encoding.png}
\caption{\label{fig:enc}(left) The gradient-trained per-spin encoding map (each spin a
distinct, frequency-selective curve) compared to $\arcsin\sqrt{s}$; (right) the encoder
validation-loss curve over 100 Adam steps.}
\end{figure}
...
MG $h{=}10$ & $\mathbf{0.00155}$ & 0.0024 & 0.0057 \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
LSTM (${\sim}1.6\times$) and the ESN (${\sim}3.7\times$), confirmed across three seeds
($0.00155\pm0.00018$, beating both baselines 3/3). The advantage thus appears
...
appears---confirmed across three seeds---on hard chaotic prediction. Next: on-device
encoding training via the parameter-shift rule; scaling past six qubits; and pushing
toward the coherence-over-decoherence regime where the full
